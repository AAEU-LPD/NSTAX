...
class NRF_BLE_DTM_SingleChannel_NRF_NRF(TestScript):
    """Test BLE DTM over a single channel.

    Both TX and RX chennels need to be NRF based devices.
    ...
    def __init__(self):
        super().__init__()
        self.name = "NRF_BLE_DTM_SingleChannel_NRF_NRF"
        self.automation_content = "NRF_BLE_DTM_SingleChannel_NRF_NRF"
        self.description = "NRF BLE DTM over a single channel"
    ...
    def teststeps(self):
        ...
        channel = test_parameters.get("channel")
        packet_type = test_parameters.get("packet_type")
        payload_length = test_parameters.get("payload_length")
        phy = test_parameters.get("phy")
        tx_power = test_parameters.get("tx_power")
        rx_sensitivity = test_parameters.get("rx_sensitivity")

        # Step 1: Start DTM TX on DUT
        step_description = "Start DTM TX on DUT"
        expected_result = "TX started successfully"
        self.logger.info("Step 1: %s", step_description)
        response = self.DUT.start_ble_dtm_tx(channel=channel, packet_type=packet_type, payload_length=payload_length, phy=phy, tx_power=tx_power)
        if response:
            actual_result = "TX started successfully"
            step_verdict = self.result_classifier.PASSED
        else:
            actual_result = "Failed to start TX"
            step_verdict = self.result_classifier.FAILED
        self.save_step(1, step_description, expected_result, actual_result, step_verdict)

        # Step 2: Start DTM RX on DUT
        step_description = "Start DTM RX on DUT"
        expected_result = "RX started successfully"
        self.logger.info("Step 2: %s", step_description)
        response = self.DUT.start_ble_dtm_rx(channel=channel, phy=phy, rx_sensitivity=rx_sensitivity)
        if response:
            actual_result = "RX started successfully"
            step_verdict = self.result_classifier.PASSED
        else:
            actual_result = "Failed to start RX"
            step_verdict = self.result_classifier.FAILED
