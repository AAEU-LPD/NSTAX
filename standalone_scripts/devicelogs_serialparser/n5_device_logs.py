...
class SerialLogger():
    def __init__(self, port, dev_name):
        self.data_filename = f'standalone_scripts/devicelogs_serialparser/TEMP_DEV_LOGS/{dev_name}_serial_data.csv'
        self.port = port
        self.dev_name = dev_name

    def connect_serial(self, duration):
        # Configure serial port
        ser = serial.Serial(self.port, 115200, timeout=1)  # Adjust COM port and baud rate as needed
        # Open CSV file for writing
        with open(self.data_filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            # writer.writerow(['Timestamp', 'Data'])  # Adjust column names as needed
            start_time = time.time()
            # Read and write serial data to CSV for defined period
            while time.time() - start_time <= duration:  # Adjust duration as needed
                ...
                data_values = data.split(',')[1:]
                if data_values:
                    writer.writerow([timestamp_utc] + data_values)  # Write timestamp and data to CSV
        # Close serial connection
        ser.close()

    def start_serial_thread(self, duration):
        # Create and start a thread for logging serial data
        self.serial_thread = threading.Thread(target=self.connect_serial, args=(duration,))
        self.serial_thread.start()

    def wait_for_serial_thread(self):
...
if __name__ == '__main__':
    ...
    converter.convert("N5_serial_data.csv", "N5_parsed_data.csv")
